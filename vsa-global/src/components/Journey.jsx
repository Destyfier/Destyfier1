...
export default function Journey() {
  ...
  return (
    <section id="journey" className="section-pad" style={{ background: 'var(--ink)', color: 'var(--paper)' }}>
      <div className="container">
        ...
        <div ref={ref} style={{ position: 'relative' }}>
          <svg
            ...
            style={{ position: 'absolute', top: 20, left: 0, overflow: 'visible' }}
          >
            <line x1="0" y1="1" x2="1000" y2="1" stroke="var(--line-on-ink)" strokeWidth="1.5" strokeDasharray="6 6" />
            <motion.line
          ...
          <div className="journey-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 24, position: 'relative' }}>
            {journey.map((step, idx) => (
              ...
              >
                ...
                <div>
                  <div className="mono" style={{ fontSize: 10.5, color: 'var(--brass-light)', letterSpacing: '0.1em', marginBottom: 6 }}>
                    GATE {step.gate}
                  </div>
                  <h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 8 }}>{step.title}</h3>
                  <p style={{ fontSize: 13.5, color: 'rgba(246,241,228,0.7)' }}>{step.body}</p>
                </div>
              </motion.div>
            ))}
          </div>
